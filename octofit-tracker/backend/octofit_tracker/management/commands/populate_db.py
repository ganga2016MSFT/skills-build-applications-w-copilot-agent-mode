...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Activity.objects.create(user=captain, workout=swim, duration=45)
        Activity.objects.create(user=batman, workout=run, duration=60)
        Activity.objects.create(user=superman, workout=swim, duration=50)
